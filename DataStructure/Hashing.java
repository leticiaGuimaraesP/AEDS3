...
public class Hashing { 
    int profundidadeGlobal=1, qtdMax = 8;
    ...
    public Hashing() throws IOException{
        ...
        pos += 4 + 4 + (qtdMax*8); //4 + 4 + (n*8) = (n = 5% da base = 200) = 1608
        diretorio.add(pos);
        arq.seek(pos);
        arq.writeInt(0); //diretorio local
        arq.writeInt(0); //quantidade de elementos
    ...
    public void writeArq(int id, int address, int bucket) throws IOException{
        ...
        if(qtdElementos<qtdMax){ //bucket com espaço livre
            ...
            if(profundidadeLocal==profundidadeGlobal){
                ...
                profundidadeGlobal++;
                for(int i=(int)Math.pow(2, profundidadeAntiga); i<Math.pow(2, profundidadeGlobal); i++){
